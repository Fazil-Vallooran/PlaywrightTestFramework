...
namespace PlaywrightTestFramework.Tests
{
    ...
    public abstract class BrowserBaseTest : PageTest
    {
        ...
        public void SetUp()
        {
            // Log configuration once per test session
            if (!_configurationLogged)
            {
                ...
                {
                    TestContext.WriteLine($"Configuration error: {ex.Message}");
                }
            }
        ...
        public void TearDown()
        {
            ...
            if (result.Outcome.Status == NUnit.Framework.Interfaces.TestStatus.Failed)
            {
                ...
                {
                    ReportPortalHelper.LogError("<span style='color: #FF0000; font-weight: bold; background-color: #FFE6E6; padding: 5px 10px; border: 2px solid #FF0000; border-radius: 5px;'>⚠️ CRITICAL TEST FAILURE - IMMEDIATE ATTENTION REQUIRED!</span>");
                }
            }
